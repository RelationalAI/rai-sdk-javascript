...
async function request(url, options = {}) {
  ...
  const fullUrl = options.query && Object.keys(options.query).length > 0 ? `${url}?${stringify(options.query, {
    arrayFormat: 'none'
  })}` : url;
...
class TransactionApi extends Base {
  async runTransaction(database, engine, transaction, mode = TransactionMode.OPEN) {
    const query = {
      dbname: database,
      compute_name: engine,
      open_mode: mode,
...
class EngineApi extends Base {
  async createEngine(name, size = EngineSize.XS) {
...
export { Client, ClientCredentials, Credentials, DatabaseState, EngineSize, EngineState, GetTokenCredentials, Permission, SdkError, TransactionMode, UserRole, UserStatus, VERSION, readConfig };
//# sourceMappingURL=rai-sdk-javascript.modern.js.map
